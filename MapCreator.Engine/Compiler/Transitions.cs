...
namespace MapCreator.Engine.Compiler
{
        ...
        public Transition(XmlElement xmlInfo)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(this.m_Description))
                    this.m_Description = "Unnamed Transition";
            ...
            {
                // Only show a simple error message without the XML content to avoid the warning popup
                throw new Exception("Error loading transition");
            }
        }
        public Transition()
        {
    ...
    public class TransitionTable
    {
        ...
        public void Load(string iFilename)
        {
            ...
            try
            {
                xmlDocument = FacetData.LoadXml(iFilename);
                ...
                try
                {
                    while (enumerator.MoveNext())
                    {
                        ...
                        {
                            // Silently skip invalid transitions to prevent warning popups
                            // but continue loading other valid transitions
                        }
                    }
                }
                finally
        ...
        {
            // XML now lives INSIDE the Engine DLL. Instead of walking a disk
            // folder, we ask FacetData for every embedded Transitions file
            // (subfolders included). Custom overrides are handled automatically.
            foreach (string relative in FacetData.EnumerateXml("Transitions", true))
            {
                this.Load(relative);
            }
        }
    ...
    public class RandomStatics : CollectionBase
    {
        ...
        public RandomStatics(string iFileName)
        {
            ...
            try
            {
                ...
                xmlDocument = FacetData.LoadXml("TerrainTypes/" + iFileName);
    ...
    public class RoughEdge
    {
        ...
        public RoughEdge()
        {
            ...
            try
            {
                ...
                xmlDocument = FacetData.LoadXml("RoughEdge/Corner.xml");
                try
                {
            ...
            try
            {
                ...
                xmlDocument = FacetData.LoadXml("RoughEdge/Left.xml");
                try
                {
            ...
            try
            {
                ...
                xmlDocument = FacetData.LoadXml("RoughEdge/Top.xml");
                try
                {
