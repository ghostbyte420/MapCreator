...
namespace MapCreator.Engine.UltimaSDK
{
    public sealed class Art
    {
        ...
        static Art()
        {
            // Size cache/removed to the FileIndex capacity (0x10000), not the physical
            // .mul length. Land IDs are masked with 0x3FFF, so slot 0x3FFF (16383) must
            // exist. Sizing to the file length left it one slot short -> IndexOutOfRange.
            int slots = m_FileIndex.Index.Length;
            m_Cache = new Bitmap[slots];
            m_Removed = new bool[slots];
        }
